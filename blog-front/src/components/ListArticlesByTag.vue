...
export default {
    ...
    methods: {  // 事件处理器
        ...
        handleListArticles: function(){
            var that = this;
            that.tagId = that.$route.query.id==undefined?1:parseInt(that.$route.query.id);  // 获取传参的 id
            var offset = (this.currentPage - 1) * this.pageSize;

            //获取详情接口
            describeArticles(null, null, that.tagId, null, offset, this.pageSize).then(response => {
