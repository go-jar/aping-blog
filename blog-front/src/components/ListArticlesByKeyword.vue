...
export default {
    ...
    methods: {  // 事件处理器
        ...
        handleListArticles: function(){
            var that = this;
            var keyword = that.$route.query.keyword==undefined? "": that.$route.query.keyword;  // 获取传参
            var offset = (this.currentPage - 1) * this.pageSize;
            console.log(keyword)
            //获取详情接口
            describeArticles(null, that.categoryId, null, keyword, offset, this.pageSize).then(response => {
