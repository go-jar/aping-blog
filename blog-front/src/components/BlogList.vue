...
<template>
    <el-row class="sharelistBox">
        <div v-if="this.$route.name=='BlogList'&&!this.$route.query.keywords" class="shareTitle">
            <div class="ui label" >
                <a  :href="'#/BlogList?classId='+classId">{{className}}</a>
            </div>
            <ul v-if="secondClass" class="shareclassTwo" >
                <li v-for="(citem, index) in secondClass" :key='index'>
                    <a :href="'#/BlogList?classId='+classId+'&classtwoId='+citem.class_id" :class="citem.class_id==classtwoId?'active':''">{{citem.cate_name}}</a>
                </li>
            </ul>
        </div>
        <el-col :span="24" class="s-item tcommonBox" v-for="(item,index) in articleList" :key="'article'+index">
            ...
            <header>
                <h1>
                    <a :href="'#/BlogDetail?aid='+item.id" target="_blank">
                        {{item.title}}
                    </a>
                </h1>
                <h2>
                ...
                <div class="ui label">
                    <a :href="'#/BlogList?classId='+item.class_id">{{item.cate_name}}</a>
                </div>
            </header>
            ...
            <div class="viewdetail">
                <a class="tcolors-bg" :href="'#/BlogDetail?aid='+item.id" target="_blank">
                    阅读全文>>
                </a>
            </div>
        </el-col>
...
<script>
import {ShowArticleAll,ArticleClassData,initDate} from '../utils/server.js'
    export default {
        name: 'BlogList',
        data() { //选项 / 数据
            return {
                ...
                for(var i=0;i<that.shareClass.length;i++){
                    if(that.classId==that.shareClass[i].class_id){
                        ...
                        if(that.shareClass[i].ChildsSon&&that.shareClass[i].ChildsSon.length>0){
                            that.secondclassList = that.shareClass[i].ChildsSon;
                        }else{
                            that.secondlassList = '';
                        }
                    }
